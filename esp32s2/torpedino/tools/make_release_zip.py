...
Output: torpedino-s2.zip i samme mappe (overskrives hvis den finnes).
"""
...
out_path = os.path.join(base, "torpedino-s2.zip")
with zipfile.ZipFile(out_path, "w", zipfile.ZIP_DEFLATED) as zf:
    for f in files:
        zf.write(os.path.join(base, f), f"torpedino/{f}")

size = os.path.getsize(out_path)
print(f"torpedino-s2.zip  –  {size:,} bytes  ({len(files)} filer)")
